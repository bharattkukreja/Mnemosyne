...
def safe_log_message(role: str, content: str) -> None:
    ...
    try:
        ...
        log_entry = {"timestamp": timestamp, "role": role, "content": str(content)}
...
def log_message(role: str, message: str) -> str:
    """Log a message from user or assistant to Nemo's memory."""
...
def log_exchange(user_message: str, assistant_message: str) -> str:
    """Log both user and assistant messages to Nemo's memory."""
...
def get_recent_messages(limit: int = 5) -> str:
    """Get recent messages as formatted text from Nemo's memory."""
    try:
        messages = []
        if LOG_FILE.exists():
...
@mcp.tool()
def clear_log() -> str:
    """Clear the conversation log from Nemo's memory."""
    try:
...
def search_memory(query: str, limit: int = 5) -> str:
    """
    Search Nemo's memory for messages containing the query string.
    ...
    try:
        ...
        with open(LOG_FILE, "r", encoding="utf-8") as f:
            lines = f.readlines()
            for line in reversed(lines):  # search backwards (most recent first)
                if query.lower() in line.lower():
                    entry = json.loads(line)
                    matches.append(f"[{entry['role']}] {entry['content']}")
